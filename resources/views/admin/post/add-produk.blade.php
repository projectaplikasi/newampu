    ...
    <div class="flex justify-center items-center">
        <div class="bg-white w-full">
            <div class="flex justify-between p-3 px-6 bg-blue-500 w-full items-center mb-4">
                <h1 class="text-xl font-semibold text-white">Tambah Produk</h1>
                <a href="/admin/dataproduk" class="text-white hover:underline">Kembali</a>
            </div>
            
            <form action="/addproduk" method="POST" enctype="multipart/form-data" class="p-8 w-4/5">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" rows="3" value=""
                        class="w-full p-1 rounded bg-gray-200 border border-gray-300 focus:outline-none focus:border-blue-500">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
